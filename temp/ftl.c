...
/*Record File*/
const char* fileName1 = "SSD_Init.txt";
const char* fileName2 = "SSD_Write.txt";
...
static void ssd_init_nand_sublock(struct nand_sublock *sublk, struct ssdparams *spp, int id, uint64_t cid, uint64_t lid, uint64_t pid, uint64_t bid)
{
    ...
    sublk->have_invalid_sensitive_page = FTL_FALSE;
}
...
static void Print_SSD_State(const char *fileName)
{
...
void ssd_init(FemuCtrl *n)
{
    ...
    Print_SSD_State(fileName1);
    printf("Init Finish \n");
}

static inline int valid_ppa(struct ssd *ssd, struct ppa *ppa)
{
    struct ssdparams *spp = &ssd->sp;
    int ch = ppa->g.ch;
    int lun = ppa->g.lun;
    ...
    if (ch >= 0 && ch < spp->nchs && lun >= 0 && lun < spp->luns_per_ch && pl >=
        ...
        && pg >= 0 && pg < spp->pgs_per_sublk && sec >= 0 && sec < spp->secs_per_pg)
        return FTL_TRUE;
...
static int do_gc(struct ssd *ssd, struct nand_block *secure_deletion_blk, int need_secure_deletion)
{
    ...
    if (need_secure_deletion == FTL_TRUE){
    ...
    for (int i=0; i<sublks_per_blk; i++){
        ...
        if (victim_sublk->state == Victim || victim_sublk->have_invalid_sensitive_page == FTL_TRUE){
            if (victim_sublk->have_invalid_sensitive_page == FTL_TRUE){
...
static void ssd_trim(struct ssd *ssd, NvmeRequest *req)
{
    ...
    int need_do_secure_deletion = FTL_FALSE;
    ...
    for (uint64_t lpn=start_lpn; lpn<=end_lpn; lpn++){
        ...
        if (mapped(ssd, lpn) == MAPPED){
            ...
            if (pg->type == Sensitive){
                need_do_secure_deletion = FTL_TRUE;
            ...
            if (pg->type == Sensitive){
                sublk->have_invalid_sensitive_page = FTL_TRUE;
            }
    ...
    // 執行secure deletion
   if (need_do_secure_deletion == FTL_TRUE){
...
static uint64_t ssd_read(struct ssd *ssd, NvmeRequest *req)
{
    ...
    for (lpn = start_lpn; lpn <= end_lpn; lpn++) {
        ...
        if (mapped(ssd, lpn) == UNMAPPED || valid_ppa(ssd, ppa) == FTL_FALSE) {
...
static uint64_t ssd_write(struct ssd *ssd, NvmeRequest *req)
{
   ...
   struct ppa *secure_deletion_table = (struct ppa *)malloc(len * sizeof(struct ppa));
    ...
    while (Enforce_do_gc() == FTL_TRUE){
        // printf("2299\n");
        int r2 = do_gc(ssd, NULL, FTL_FALSE);
        // printf("2336\n");
        ...
        if (r2 == Stop_GC){
            ...
            if (Check_whether_enforce_clean_blcok() == FTL_TRUE){
                //printf("2342\n");
